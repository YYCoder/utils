...
(function(global, factory) {
    ...
    /**
     * 安全获取对象深层属性
     * @param  {Array|String|Number}   props   [要获取的属性列表]
     * @param  {Object}  obj                   [获取属性的对象]
     * @param  {Any}  defaultValue             [默认值，当结果为 null 时，会返回该值]
     * @return {Any}                           [获取到的属性值，或null]
     */
    prototype['safeGetter'] = function(props, obj, defaultValue) {
        var keys = null
        if (Array.isArray(props)) keys = props
        else if (typeof props === 'string') {
            var propsStr = props.replace(/\[([^\[\]]+)\]/g, '.$1')
            keys = propsStr.split('.')
        }
        else if (typeof props === 'number') keys = [props]
        else {
            throw new Error('[utils Error]: safeGetter arguments must be one of these types: array/string/number, now got ' + typeof (props))
        }
        if (!prototype['isObject'](obj)) throw new Error('[utils Error]: safeGetter second argument must be Object')

        // 获取结果：若结果是 null，则返回传入的默认值
        var res = keys.reduce(function(pre, next) {
            return (pre && pre[next]) ? pre[next] : null
        }, obj)
        if (prototype['isNull'](res)) return defaultValue
        return res
    }
    /**
     ...
     * @return {Boolean}           [鉴定结果]
     */
    prototype['isUndefined'] = function(arg) {
