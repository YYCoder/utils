...
;(function(global, factory) {
    ...
    if (elem instanceof Element) {
      ...
      if (typeof selector === 'string') {
        while (parent !== document.documentElement) {
          isMatched =
            (parent.matchesSelector && parent.matchesSelector(selector)) ||
  ...
  function isIntoView(elem) {
    ...
    if (typeof elem.getBoundingClientRect === 'function') {
      ...
      isVIntoView =
        position['top'] <= screen.availHeight && position['bottom'] >= 0
      isHIntoView =
        position['left'] <= screen.availWidth && position['right'] >= 0
    } else {
  ...
  function isVerticalIntoView(elem, container) {
    ...
    if (!container || container == window) {
      ...
      isVerticalIntoView =
        position['top'] <= windowBottom && position['bottom'] >= windowTop
    } else {
      position = getPosition(elem, container)
      scrollTop = container.scrollTop
      scrollBottom =
        scrollTop + window.parseInt(window.getComputedStyle(container).height)
      isVerticalIntoView =
        position['top'] <= scrollBottom && position['bottom'] >= scrollTop
    }
  ...
  function isHorizontalIntoView(elem, container) {
    ...
    if (!container || container == window) {
      ...
      windowRight =
        windowLeft + (window.innerWidth || document.documentElement.clientWidth)
      isHorizontalIntoView =
        position['left'] <= windowRight && position['right'] >= windowLeft
    } else {
      position = getPosition(elem, container)
      scrollLeft = container.scrollLeft
      scrollRight =
        scrollLeft + window.parseInt(window.getComputedStyle(container).width)
      isHorizontalIntoView =
        position['left'] <= scrollRight && position['right'] >= scrollLeft
    }
  ...
  function getPosition(elem, container) {
    var parent = container instanceof Element ? container : null
    var position = {}
    position['top'] = 0
